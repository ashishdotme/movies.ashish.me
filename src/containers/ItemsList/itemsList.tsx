...
class ItemsList extends React.Component<ItemsListProps, State> {
  ...
  public handleSortOrder = () => {
    this.setState({
      sortOrder:
        this.state.sortOrder === "Descending" ? "Ascending" : "Descending"
    });
  };
  ...
  public getFilteredItems = (results: Movie[]) => {
    ...
    if (selectedGenre.key !== "-1") {
      results = results.filter(item =>
        item.genre.includes(selectedGenre.value)
      );
    }
    if (selectedWatchedYear.key !== "-1") {
      results = results.filter(
        item =>
          moment(item.viewingDate)
            .year()
            .toString() === selectedWatchedYear.value
      );
    }
  ...
  render() {
    ...
    if (this.props.match.params.id && Number(this.props.match.params.id) > 0) {
      if (
        Number(this.props.match.params.id) * 12 - 11 <=
    ...
    let slicedResults = filteredResults.slice(
      (currentPage - 1) * 12,
      currentPage * 12
    );
            ...
            <div className="container">
              ...
              {filteredResults.length > 12 && (
                <Pagination
                  ...
                  onChange={(page: any) =>
                    this.handlePageChange(page, filteredResults)
                  }
